...
@section('content')
    ...
    <div class="row">
        <div class="col-xl-9">
            <div class="card">
                        ...
                        <table class="table table-nowrap align-middle table-borderless mb-0">
                            <thead class="table-light text-muted">
                                <tr>
                                    ...
                                    <th scope="col">@lang('translation.service_appointment')</th>
                            ...
                            <tbody>
                                @foreach ($row->orderServices as $service)
                                    <tr>
                                        ...
                                        <td>{{ $service->appointment_date }} {{ $service->appointment_time ? \App\Enums\AppointmentTimes::getStatus($service?->appointment_time) : null }}</td>
                ...
                @foreach ($row->orderServices as $service_details)
                            @php
                                $fields = $row->transaction->orderServiceFields->where('service_id', $service_details->service_id);
                            @endphp

                            @if($fields->isNotEmpty())
                                <div class="card">
                                    <div class="card-header">
                                        <h6>تفاصيل طلب خدمة ({{ $service_details?->service?->name }})</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="table-responsive table-card">
                                            <table class="table table-nowrap align-middle table-border mb-0">
                                                <thead class="table-light text-muted">
                                                    <tr>
                                                        <th scope="col">خيارات الخدمة</th>
                                                        <th scope="col">اختيارات المزارع</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($fields as $field)
                                                        <tr>
                                                            <td>{{ $field->key }}</td>
                                                            <td>{{ !is_null($field->value) ? json_decode($field->value) : '' }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                                ...
                                <form action="{{ route('vendor.service_change_shipping_status') }}" method="post"
                                    ...
                                    <select id="orderStatus" name="shipping_status" class="form-control text-center"
                                        ...
                                        @if ($row->status == 'registered')
                                            ...
                                            @if (isset($row->orderService) && $row->orderService->appointment_date)
                                                @php
                                                    $date = \Carbon\Carbon::parse($row->orderService->appointment_date);
                                                @endphp
                                                @if ($date->lessThanOrEqualTo(\Carbon\Carbon::today()))
    ...
    @if ($row->status == 'registered' && isset($row->orderService) && $row->orderService->appointment_date)
        @php
            $date = \Carbon\Carbon::parse($row->orderService->appointment_date);
        @endphp
        @if ($date->lessThanOrEqualTo(\Carbon\Carbon::today()))
            <div class="modal fade position-fixed" id="exampleModalScrollable" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" id="form-package" role="document">
