...
class ReportController extends Controller
{
    ...
    public function SalesAllVendors(Request $request)
    {
       ...
       // Get queries for both orders and order services
        $ordersQuery = $this->getVendorSalesQuery($request);
        $orderServicesQuery = $this->getVendorServiceSalesQuery($request);


        // Fetch the data from both queries
        $getOrders = $ordersQuery->get();
        $getOrderServices = $orderServicesQuery->get();

        // Combine the results
        $orders = $this->combineOrdersAndServices($getOrders, $getOrderServices);

        if (isset($request->all_export_excel) && $request->all_export_excel) {
            return Excel::download(new SalesAllVendorsExport($orders), 'sales_vendors '.Date('d-m-Y').Str::random(1).'.xlsx');
        }

        return view('admin.new-reports.SalesAllVendors', compact('orders','vendors'));
    }

    public function combineOrdersAndServices($orders, $orderServices)
    {
        $combinedResults = collect();

        // Create a map for orders by vendor_id
        $ordersMap = $orders->keyBy('vendor_id');

        foreach ($orderServices as $service) {
            if ($ordersMap->has($service->vendor_id)) {
                // Combine the order and service totals for the same vendor
                $ordersMap[$service->vendor_id]->total_without_vat += $service->total_without_vat;
                $ordersMap[$service->vendor_id]->total_vat += $service->total_vat;
                $ordersMap[$service->vendor_id]->total_with_vat += $service->total_with_vat;
                $ordersMap[$service->vendor_id]->value_of_company_profit_vat += $service->value_of_company_profit_vat;
                $ordersMap[$service->vendor_id]->total_company_profit_without_vat += $service->total_company_profit_without_vat;
                $ordersMap[$service->vendor_id]->total_company_profit += $service->total_company_profit;
                $ordersMap[$service->vendor_id]->total_balance += $service->total_balance;
            } else {
                // If the vendor is only in order services, add it to the combined results
                $ordersMap->put($service->vendor_id, $service);
            }
        }

        // Add all order entries to the combined results
        $combinedResults = $ordersMap->values();
    ...
    public function SalesAllVendorsPrint(Request $request  , $order_id = null)
    {
        ...
        if (!empty($order_id)) {
            $ordersQuery->where('id', $order_id);
            $orderServicesQuery->where('id', $order_id);
        }

        // Fetch the data from both queries
        $orders = $ordersQuery->get();
        $orderServices = $orderServicesQuery->get();

        // Combine the results
        $getOrders = $this->combineOrdersAndServices($orders, $orderServices);
    ...
    public function getVendorServiceSalesQuery($request){
        $orders_query = OrderService::query()->with('vendor.wallet')
        ->select('vendor_id' , 'id' , 'wallet_id', DB::raw('SUM(sub_total) as total_without_vat,
         SUM(vat) as total_vat,
         SUM(total) as total_with_vat,
         ROUND((SUM(company_profit) - (SUM(company_profit) / 1.15)),2) as value_of_company_profit_vat,
         ...
         ROUND(SUM(vendor_amount),2) as total_balance
         '
         ))->whereStatus(ServiceOrderStatus::COMPLETED)->groupBy('vendor_id')
         ->when($request->get('from') && $request->get('to'),function($query){
            $query->whereHas('vendorWalletTransactions',function($qr2){
                $qr2->createdBetween()->completed();
            });
    ...
    public function vendors_sales(Request $request)
    {
        ...
        $orders =  VendorWalletTransaction::with(['referenceOrder','wallet.vendor'])->completed()->filterVendor()->createdBetween()->latest();
    ...
    {
        $rows =  VendorWalletTransaction::with(['referenceOrder','wallet.vendor'])->completed()->filterVendor()->createdBetween()->latest();

        $orders = $rows->get();
        $sub_total_in_sar_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->sub_total_in_sar_rounded  * 100;
        });
        $vat_in_sar_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->vat_in_sar_rounded  * 100;
        });
        $total_in_sar_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->total_in_sar_rounded  * 100;
        });
        $company_profit_without_vat_in_sar_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->company_profit_without_vat_in_sar_rounded  * 100;
        });
        $company_profit_vat_rate_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->company_profit_vat_rate_rounded  * 100;
        });
        $company_profit_in_sar_rounded = $orders->sum(function ($operation) {
            return $operation->referenceOrder?->company_profit_in_sar_rounded  * 100;
        });
