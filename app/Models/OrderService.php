...
class OrderService extends Model implements HasMedia
{
    ...
    {
        return $this->hasOne(AssignOrderServiceRequest::class, 'order_service_id')->latestOfMany();
    }
    ...
    {
        return $this->hasOne(OrderServiceDetail::class, 'order_service_id')->latestOfMany();
    }
    ...
    {
        return $this->hasOne(OrderProcessRate::class, 'order_service_id');
    }
    ...
    public function vendorWalletTransactions()
    {
        return $this->hasMany(VendorWalletTransaction::class, 'reference_id')
            ->where('vendor_wallet_transactions.reference', OrderService::class);
    }

    public function isAdminCanCancel(){
        return !in_array($this->status,['completed','canceled']) && in_array($this->transaction->payment_method,[2,3]) && auth()->user()?->isAdminPermittedTo('admin.service_transactions.cancel');
    }

    public function isAdminCanRefund(){
        return $this->status == 'completed' && in_array($this->transaction->payment_method,[2,3]) && $this->delivered_at > now()->subDays(3) && auth()->user()?->isAdminPermittedTo('admin.service_transactions.cancel');
    }
