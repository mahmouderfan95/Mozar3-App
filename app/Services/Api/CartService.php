...
class CartService
{
    ...
    private function checkCapacity($service, $appointmentDate, $timeSlot)
    {
        ...
        $serviceAppointment = ServiceAppointment::where('service_id', $service->id)
            ->where('time_slot', $timeSlot)
            ->first();

        if ($serviceAppointment && $serviceAppointment->capacity <= $ordersCount) {
            return response()->json([
                'success' => false,
                'status' => 400,
                'data' => [],
                'message' => __('cart.api.no_available_capacity')
            ], 400);
        }
    ...
    private function checkAppointmentDays($service, $appointmentDate)
    {
    ...
    public function addServicesToCart($request)
    {
            ...
            // Check appointment days
            $appointmentDaysResponse = $this->checkAppointmentDays($service, $appointmentDate);
            if ($appointmentDaysResponse) {
                return $appointmentDaysResponse;
            }

            // Check capacity
            $capacityResponse = $this->checkCapacity($service, $appointmentDate, $appointmentTime);
            if ($capacityResponse) {
                return $capacityResponse;
            }
            $cart = $this->getCurrentUserActiveServicesCartOrCreate($request);

            #check cart is exist one service
            if($cart->cartProducts()->whereNull('product_id')->where('service_id','!=',$service->id)->count() > 0){
                return response()->json([
                    'success' => false,
                    'status' => 400,
                    'data' => [],
                    'message' => __('cart.api.service_exists_in_vendor_cart')
                ],400);
            }
            #check cart is product exist
    ...
    public function getCurrentUserActiveServicesCartOrCreate($request)
    {
        ...
        if(auth('api_client')->check())
            ...
            if(auth('api_client')->user())
            {
                ...
                if (!$cart) $cart = Cart::create(['user_id' => $auth, 'is_active' => 1 , 'city_id' => request()->city_id,'service_address' => request()->serviceAddress,'service_date' => request()->serviceDate,'type' => 'order-service'] );
                ...
                $cart = Cart::where('guest_customer_id', $guest->id)->first();
                if (!$cart) $cart = Cart::create(['guest_customer_id' => $guest->id, 'is_active' => 1 , 'city_id' => request()->city_id,'service_address' => request()->serviceAddress,'service_date' => request()->serviceDate,'type' => 'order-service'] );
                else if ($cart->is_active != 1) $cart->update(['is_active' => 1]);
                else if(request()->has('city_id')  && $cart->city_id != request()->get('city_id') )  $cart->update(['city_id' => $request['city_id']]);
                else if(request()->has('serviceAddress')  && $cart->service_address != request()->get('serviceAddress') )  $cart->update(['service_address' => request()->serviceAddress]);
                else if(request()->has('serviceDate')  && $cart->service_date != request()->get('serviceDate') )  $cart->update(['service_date' => request()->serviceDate]);
    ...
    public function getCartProductGroupedByVendor(int $customerId, Closure $totalsCalculation = null , $type): array
    {
        ...
        // $cart =Cart::where('user_id',auth('api_client')->user()->id)->where('type','order')->first();
    ...
    public function reorderServices(int $transaction_id,$request)
    {
        $transaction = $this->transactionRepository->getModelTypeOfOrderService($transaction_id);
        ...
        // loop on order services
        foreach($transaction->orderServices as $orderService)
        {
            // loop on order details
            foreach($orderService->orderServices as $orderDetail)
            {
                $service = Service::where('id',$orderDetail->service_id)->Available()->first();
                if($service){
                    $this->newSyncServiceCart($transaction,$orderDetail,$service,$cart);
                }else{
                    return [
                        'success' => false,
                        'status' => 400,
                        'data' => [],
                        'message' => __('cart.api.service_not_available')
                    ];
                }
            }
        }
        return [
            'success' => true,
            'status' => 200,
            'data' => $this->getCartServiceGroupedByVendor(),
            'message' => __('cart.api.reorder_successfully')
        ];

    }
    private function newSyncServiceCart($transaction,$orderDetail,$service,$cart,$append = false)
    {
        if($orderDetail->quantity < 1) return false;

        #get cartProduct
        $cartProduct = $cart->cartProducts()->where('service_id',$service->id)->first();
        #create
        if(!$cartProduct) return $this->_syncAddServicesToCart($transaction,$orderDetail, $cart);

        #checkDuplicate
        $checkDuplicate = $cart->cartProducts()->where('service_id',$orderDetail->service_id)->count();
        if($checkDuplicate > 1) $cart->cartProducts()->where('service_id', $orderDetail->service_id)->where('id','!=',$cartProduct->id)->delete();

        #update
        return $this->_syncEditServicesInCart($transaction,$orderDetail, $cartProduct, $cart,$append);
    }
    ...
    private function _syncAddServicesToCart($transaction,$orderDetail,$cart)
    {
        if ($cart == null || $orderDetail->quantity < 1) return null;
        $service =Service::findOrFail($orderDetail->service_id);
        $totalPrice = 0;
        // calc service price
        if($transaction->orderServiceFields){
            foreach($transaction->orderServiceFields as $field){
                $price = ServiceField::where('service_id',$service->id)
                ->where('field_name',$field['key'])
                ->where('field_value',json_decode($field['value']))
                ->sum('field_price');
                $totalPrice+=$price;
            }
        }
        $cartProduct = $this->repository->createCartProductServices($cart,[
            ...
            'total_price' => ($totalPrice * $orderDetail->quantity),
            'appointment_date,' => $orderDetail->appointment_date,
            'appointment_time,' => $orderDetail->appointment_time,
        ]);
        $serviceCartProductCartProduct=CartProduct::where('cart_id',$cart->id)->get();
        foreach($transaction->orderServiceFields as $fields)
        {
            foreach($serviceCartProductCartProduct as $ser){
                CartProductServiceField::create([
                    'cart_product_id' => $ser['id'],
    ...
    private function _syncEditServicesInCart(
        $transaction,
        $orderDetail,
        CartProduct $cartProduct,
        Cart         $cart,
    ...
    )
    {
        ...
        // calc service price
        // $data = CartProductServiceField::where('service_id',$request['service_id'])->where('cart_product_id',$cartProduct->id)->get();
        // dd($transaction->orderServiceFields);
        if($transaction->orderServiceFields){
            foreach($transaction->orderServiceFields as $field){
                $price = ServiceField::where('service_id',$service->id)
                ->where('field_name',$field['key'])
