...
class TransactionController extends ApiController
{
    ...
    public function saveOrderServicesRate(int $transaction_id,TransactionRateServiceStore $request)
    {
        $response = $this->service->saveOrderServicesRate($transaction_id, $request);
        return $this->setApiResponse(
                $response['success'],
                $response['status'],
                $response['data'],
    ...
    public function OrderServiceVendorFeedback(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        $order = OrderService::where('transaction_id',$transaction->id)->find($request->order_service_id);
        if(!$order){
            return response()->json(['status'=> false,'message' => 'طلب غير موجود!'])->setStatusCode(404);
        }
