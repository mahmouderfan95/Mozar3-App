...
use App\Events\ServiceTransaction\Created;
...
class TransactionRepository extends BaseRepository
{
    ...
    private function checkCapacity($service, $appointmentDate, $timeSlot)
    {
        ...
        if ($serviceAppointment && $serviceAppointment->capacity <= $ordersCount) {
            return ['success'=>false,'message'=>__('cart.api.no_available_capacity')];
        }
    ...
    private function checkAppointmentDays($service, $appointmentDate)
    {
        ...
        if (!in_array($dayOfWeek, $appointmentDays)) {
            return ['success'=>false,'message'=>__('cart.api.selected_date_not_available')];
        }
    ...
    public function serviceCheckout($request)
    {
        ...
        if($cart->cartProducts->count() > 0){
            foreach($cart->cartProducts as $item){
                ...
                $appointmentDate = Carbon::parse($item->appointment_date);
    ...
    public function serviceOrdering($cart,$userWallets,$payment_id,$request,$PayTransaction = null)
    {
        ...
        try {
                ...
                foreach ($cartProducts as $key => $cartProduct) {
                    OrderServiceDetail::updateOrCreate([
                        ...
                        [
                            ...
                            'vat_percentage' => $order->vat_percentage,
                            'appointment_date' => $cartProduct->appointment_date,
                            'appointment_time' => $cartProduct->appointment_time,
                        ]);
                    $num_products += $cartProduct->quantity;
                }
            ...
            #sms to client
            ClientMessageService::transactionServiceCreatedMessage(ClientMessageEnum::CreatedServiceTransaction , $transaction);
    ...
    public function paymentCallback(Request $request)
    {
        ...
        if ($request->has('TrackId')) {
            ...
            return $cart->type == 'order-service' ? $this->serviceOrdering($cart, $userWallets, PaymentMethods::VISA, $request, $UrwayTransaction) : $this->ordering($cart, $userWallets, PaymentMethods::VISA, $request, $UrwayTransaction);
        }
        // TABBY
        elseif($request->has('payment_id')){
            ...
            #ordering after validations
            return $cart->type == 'order-service' ? $this->serviceOrdering($cart, $userWallets, PaymentMethods::TABBY, $request, $TabbyTransaction) : $this->ordering($cart, $userWallets, PaymentMethods::TABBY, $request, $TabbyTransaction);
        }
