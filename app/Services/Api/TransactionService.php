...
use App\Http\Requests\Api\TransactionRateServiceStore;
...
class TransactionService
{
    ...
    public function saveOrderServicesRate(int $transactionId,TransactionRateServiceStore $request)
    {
        $transaction = Transaction::query()
            ->customer(auth('api_client')->user()->id)
            ->id($transactionId)
            ...
            ->with([
                'orderServices' => fn($orderQuery) => $orderQuery->with([
                    ...
                    'services' => function ($productsQuery) use ($transactionId) {
                        $productsQuery
                            ->with([
                                'reviews' => fn($reviewsQuery) => $reviewsQuery
                                    ->customer(auth('api_client')->user()->id)
        ...
        foreach ($request->vendors as $vendorData) {
            $order = $transaction->orderServices->where('vendor_id', $vendorData['id'])->first();
            if (!$order) continue; // ignore non exists orders
            ...
            foreach ($vendorData['services'] as $serviceData) {
                $serviceOrder = Service::where('id', $serviceData['id'])->first();
                if (!$serviceOrder || (!$serviceData['rate'] && !isset($serviceData['review']))) continue; // ignore non exists services
                if ($serviceOrder->reviews->isEmpty()) {
                    $serviceOrder->reviews()->create([
                        'user_id' => auth('api_client')->user()->id,
                        'rate' => $serviceData['rate'],
                        'comment' => $serviceData['review'] ?? "",
                        'transaction_id' => $transactionId
                    ]);
                }
            }
