...
class ServiceRepository{
	...
	public function store($data)
	{
        ...
        // Check if appointment_time and appointment_capacity exist and are arrays
        if (isset($data['appointment_time']) && is_array($data['appointment_time']) &&
        isset($data['appointment_capacity']) && is_array($data['appointment_capacity'])) {

            $appointments = $data['appointment_time'];
            $capacities = $data['appointment_capacity'];

            foreach ($appointments as $index => $time) {
	...
	public function update($data,$id)
	{
        ...
        if ($row->status == 'accepted') {
            ...
            $temp_data = $data;
            unset($temp_data['cities'],$temp_data['appointment_time'],$temp_data['appointment_capacity']);
        ...
        isset($data['appointment_capacity']) && is_array($data['appointment_capacity'])) {
            ...
            ServiceAppointment::where('service_id', $row->id)->delete();
	...
	private function createServiceTemp($service,$data)
	{
		$serviceData=[
            ...
            'appointment_days'=>$data['appointment_days'],
	...
	{
		$tempData = array_diff_key(json_decode($temp->data,true), array_flip(["name", "desc", "conditions", "images_array","deleted_images_array","formAction","_method","_token","appointment_days","service_provider_phone"]));
		...
		$serviceOldData = $serviceOldData->setHidden(['vendor','created_at','updated_at','service_temp','name','desc','conditions','appointment_days','service_provider_phone'])->toArray();
        ...
        if ($service->appointment_days != $temp->appointment_days) {
            $diff['appointment_days']=$temp->appointment_days;
        }

        if ($service->service_provider_phone != $temp->service_provider_phone) {
            $diff['service_provider_phone']=$temp->service_provider_phone;
        }
