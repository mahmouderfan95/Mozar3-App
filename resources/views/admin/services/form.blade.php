<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body">
                <div class="col-md-12 mb-3">
                    <label for="cities" class="form-label @error('cities') is-invalid @enderror">@lang('admin.cities.all_cities') <span
                            class="text-danger">*</span></label>
                    <select class="form-control" name="cities[]" id="cities" data-choices data-choices-removeItem
                        multiple>
                        <option value="">@lang('admin.select')</option>
                        @foreach ($cities as $city)
                            <option @if (isset($row) && in_array($city->id, $row->cities->pluck('id')->toArray())) selected @endif value="{{ $city->id }}">
                                {{ $city->name }}</option>
                        @endforeach
                    </select>
                    @error('cities')
        ...
        <div class="card">
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label" for="service_provider_phone">@lang('translation.service_provider_phone')
                        <span class="required" style="color: red!important">*</span>
                    </label>
                    <input type="text" class="form-control @error('service_provider_phone') is-invalid @enderror"
                        name="service_provider_phone"
                        value="{{ isset($row) ? $row->service_provider_phone : old('service_provider_phone') }}"
                        placeholder="{{ __('translation.service_provider_phone_placeholder') }}" id="service_provider_phone">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">@lang('translation.service_appointments')</h5>
            </div>
            <div class="card-body">
                <div class="mb-4">
                    <h5 class="fs-14 mb-1">@lang('translation.service_appointment_days')<span class="required"
                            style="color: red!important">*</span></h5>
                    @foreach ($appointmentDays as $key => $day)
                        <label class="form-label"><input @if (isset($row) && isset($row->appointment_days) && in_array($key, $row->appointment_days)) checked @endif class="form-check-input ms-3 me-2" type="checkbox"
                                name="appointment_days[]" value="{{ $key }}">{{ $day }}</label>
                    @endforeach
                    <br>
                    @error('appointment_days')
                        <span class="error text-danger"> {{ $message }} </span>
                    @enderror
                </div>
                <div class="mb-4">
                    <h5 class="fs-14 mb-1">@lang('translation.service_appointment_time')<span class="required"
                            style="color: red!important">*</span></h5>
                            <div class="appointment-container">
                                @foreach (\App\Enums\AppointmentTimes::getTimeList() as $timeValue => $timeLabel)
                                    <div class="appointment-row">
                                        <label class="form-label">
                                            <span class="text-secondary">{{ $timeValue }}</span>
                                            <input @if (isset($row) && in_array($timeValue, $row->timeAppointments->pluck('time_slot')->toArray())) checked @endif class="form-check-input ms-3 me-2 appointment-time-checkbox" type="checkbox" name="appointment_time[]" value="{{ $timeValue }}">
                                            <span>{{ $timeLabel }}</span>
                                        </label>
                                        @php
                                            $timeAppointmentsKeyed = isset($row) ? $row->timeAppointments->pluck('capacity', 'time_slot')->toArray() : null;
                                        @endphp
                                        <input class="form-control appointment-capacity ms-2" min="1" name="appointment_capacity[]" value="{{ isset($timeAppointmentsKeyed[$timeValue]) ? $timeAppointmentsKeyed[$timeValue] : '' }}" required type="number" placeholder="الطاقة الإستيعابية" {{ !isset($timeAppointmentsKeyed[$timeValue]) ? 'disabled' : '' }}>
                                    </div>
                                @endforeach
                            </div>
                    <br>
...
@section('script-bottom')
    ...
    <script>
        $(document).ready(function() {
            $('.appointment-time-checkbox').change(function() {
                var index = $(this).closest('.appointment-row').index();
                var capacityInput = $(this).closest('.appointment-row').find('.appointment-capacity');
                if ($(this).is(':checked')) {
