            ...
            <div class="card">
                ...
                @foreach ($transaction->orderServices as $order)
                    <div class="card">
                            ...
                            <div class="table-responsive table-card">
                                <table class="table table-nowrap align-middle table-borderless mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col">@lang('admin.services.service_appointment')</th>
                                    ...
                                    @foreach ($order->orderServices as $orderService)
                                        <tr>
                                            ...
                                            <td>{{ $orderService?->appointment_date }} {{ $orderService?->appointment_time ? \App\Enums\AppointmentTimes::getStatus($orderService?->appointment_time) : null }}</td>
                        ...
                        @foreach ($order->orderServices as $service_details)
                            @php
                                $fields = $transaction->orderServiceFields->where('service_id', $service_details->service_id);
                            @endphp
                ...
                <div class="card-header">
                    <div class="d-flex">
                        <h5 class="card-title flex-grow-1 mb-0"><i
                            ...
                            ({{ $transaction->orderServices[0]->vendor->name }})
                        </h5>
                    </div>
                </div>
                <div class="card-body">
                    <div class="text-center">
                        <p><label for="">@lang('vendors.shipping_info.status')</label> :
                            {{ \App\Enums\ServiceOrderStatus::getStatus($transaction->status) }}</p>
                    </div>
                </div>
            </div>
            <div class="card">
